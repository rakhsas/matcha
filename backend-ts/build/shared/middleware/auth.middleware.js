        ...
        try {
            ...
            if ((err.name === 'TokenExpiredError' && rToken) || (err.name === 'AccessTokenNotFoundException' && rToken)) {
                try {
                    ...
                    const decryptedSub = CryptoUtil.decrypt(decoded.sub, process.env.PAYLOAD_ENCRYPTION_KEY || '');
                    const user = yield userService.findById(decryptedSub);
                    if (!user)
                    ...
                    setCookies('aToken', parseInt(process.env.ATOKEN_VALIDITY_DURATION_IN_SECONDS || '0'), newAccessToken, res);
                    req.userId = decryptedSub;
                    next();
                }
                catch (refreshError) {
...
const setCookies = (tName, duration, token, res) => {
